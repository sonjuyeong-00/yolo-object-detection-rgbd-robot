...
class RobotControllerNode(Node):
    def __init__(self):
        ...
        ## <sys>
        ## Realsense HW -> realsense2_camera ROS node -> sensor_msgs/Image
        ...
        # sys add YOLO
        self.get_logger().info("YOLO Initial : load v8n...")
        self.last_yolo_time = 0.0
        self.yolo = YOLO("yolov8n.pt")  # 가벼운 모델
        self.target_class = None        # 예: "bottle" 같은 것만 잡고 싶으면 사용
        self.last_target_uv = None
        ##
    ...
    def synced_callback(self, color_msg, depth_msg, info_msg):
        try:
            #ROS Image -> OpenCV 
            #컬러 #sensor_msgs/Image #OpenCV BGR 이미지 (numpy)
            #뎁스 #sensor_msgs/Image #OpenCV uint16 depth 이미지
            #cvBridge가 ROS <-> OpenCV 다리역할 
        ...
        # sys YOLO update
        now = time.time()
        if now - self.last_yolo_time > 0.2 and self.latest_cv_color is not None:
            self.last_yolo_time = now
            self.run_yolo_and_update_target()
...
## sys add
def run_yolo_and_update_target(self):
    img = self.latest_cv_color
    results = self.yolo.predict(img, conf=0.5, verbose=False)
    ...
    self.last_target_uv = (u, v)

    # 화면용으로 bbox 저장해두면 좋음
    self.last_bbox = (int(x1), int(y1), int(x2), int(y2), float(confs[idx]))

def get_depth_median_mm(self, u, v, k=3):
    # k=3이면 7x7 윈도우(2k+1)
    h, w = self.latest_cv_depth_mm.shape
    u0, u1 = max(0, u-k), min(w-1, u+k)
    v0, v1 = max(0, v-k), min(h-1, v+k)
    patch = self.latest_cv_depth_mm[v0:v1+1, u0:u1+1].astype(np.int32)
...
def compute_robot_xyz_from_uv(self, u, v):
    depth_mm = self.get_depth_median_mm(u, v, k=3)
    if depth_mm == 0:
        return None
    ...
    return final_x, final_y, final_z, depth_mm
...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            ...
            if robot_controller.latest_cv_color is None:
                continue

            display_image = robot_controller.latest_cv_color.copy()

            # YOLO로 검출된 중심 픽셀이 있을 때
            if getattr(robot_controller, "last_target_uv", None) is not None:
                u, v = robot_controller.last_target_uv

                # 중심점 표시
                cv2.circle(display_image, (u, v), 6, (0, 255, 0), -1)

                # bbox 표시
                if getattr(robot_controller, "last_bbox", None) is not None:
                    x1, y1, x2, y2, conf = robot_controller.last_bbox
                ...
                # 3D 좌표 계산
                if robot_controller.intrinsics is not None and robot_controller.latest_cv_depth_mm is not None:
                    result = robot_controller.compute_robot_xyz_from_uv(u, v)

                    if result is not None:
                        fx, fy, fz, depth_mm = result

                        # 화면 좌표 표기
                        cv2.putText(
                            display_image,
                            f"Pixel: (u={u}, v={v})  Depth: {depth_mm} mm",
                            (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.7,
                            (255, 255, 255),
                        ...
                        cv2.putText(
                            display_image,
                            f"Robot XYZ: X={fx:.1f}  Y={fy:.1f}  Z={fz:.1f}",
                            (10, 60),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.7,
                            (255, 255, 255),
